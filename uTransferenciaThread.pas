...
type
  TTransferenciaThread = class(TThread)
  private
...
constructor TTransferenciaThread.Create(AConexaoOrigem, AConexaoDestino: TUniConnection; AMemoLog: TMemo);
begin
...
procedure TTransferenciaThread.Log(const Msg: string);
begin
  if Assigned(FMemoLog) then
  begin
    FLogMsg := Msg;
    TThread.Synchronize(nil, LogSync);
  end;
  // Também escreve no log formatado
  FrmPrincipal.WriteLogFormatted('INFO', '1', Msg);
end;

procedure TTransferenciaThread.LogSync;
begin
  if Assigned(FMemoLog) then
    FMemoLog.Lines.Add(FLogMsg);
end;

procedure TTransferenciaThread.Execute;
var
